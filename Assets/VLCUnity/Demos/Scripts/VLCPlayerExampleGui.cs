...
{
    public VLCMediaPlayer vlcPlayer;
    ...
    void Start()
    {
        ...
        vlcPlayer.OnTextureResized += (texture) => {
            if (texture != null && screenAspectRatioFitter != null)
                screenAspectRatioFitter.aspectRatio = (float)texture.width / (float)texture.height;
        };
        ...
        //Path Input
        pathInputField.text = vlcPlayer.mediaPath;
